...
function getAdminUrl(url, type) {
  const project = url.hostname === 'localhost' ? 'main--builder-prospect-prod--sapudex' : url.hostname.split('.')[0];
...
async function getStatus(url) {
  ...
  const preview = json.edit.status === 404 ? 'N/A' : json.preview.lastModified || 'Never';
  const live = json.edit.status === 404 ? 'N/A' : json.live.lastModified || 'Never';
  const edit = json.edit.url;
  return {
    checkbox: json.edit.status === 200, url, edit, preview, live,
  };
}
...
function findLinks(selector) {
  return [...document.body.querySelectorAll(selector)]
    ...
    .filter(
      (obj, idx, arr) => arr.findIndex((item) => item.url.pathname === obj.url.pathname) === idx,
    );
}
...
function Item({ name, item, idx }) {
  const isChecked = item.checkbox && item.checked ? ' is-checked' : '';
  const isFetching = item.checkbox && !item.edit ? ' is-fetching' : '';
  const isNoCheckbox = item.checkbox ? '' : ' none';

  return html`
    <div class="preflight-group-row preflight-group-detail${isChecked}${isFetching}${isNoCheckbox}"
      onClick=${item.checkbox ? (e) => handleChange(e.target, name, idx) : undefined}>
